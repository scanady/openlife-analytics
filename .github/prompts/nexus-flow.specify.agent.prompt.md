...
- Suggested agent groupings (which tasks belong to which agents)
...
1. **Suggested Agent Groupings**: 
   - Identify 3-7 specialized agents based on domain expertise
   - List which tasks each agent handles (by task number)
   - Note if agents can operate in parallel
   - Brief rationale for grouping (1 sentence)
...
- **Intake Agent**: Tasks 1-3 (Data collection and validation)
- **Verification Agent**: Tasks 2, 4, 6 (Identity, death, fraud checks - can work in parallel)
- **Assessment Agent**: Task 5 (Complex review requiring reasoning)
- **Financial Agent**: Task 7 (Calculations and determinations)
- **Execution Agent**: Tasks 8-9 (Payment processing)
- **Communication Agent**: Tasks 10-11 (Notifications and follow-up)
...
- **Domain Expertise**: Group tasks requiring similar knowledge or skills
- **Natural Handoffs**: Agent boundaries should align with logical process transitions
...
- **Logical grouping**: Suggested agents should have coherent, related responsibilities
- **Practical boundaries**: Agent divisions should reflect real workflow handoffs
